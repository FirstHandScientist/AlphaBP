...
\aistatstitle{$\alpha$ Belief Propagation as Fully Factorized Approximation}

\aistatsauthor{ Dong Liu \And Minh Th{\`a}nh Vu \And  Zuxing Li \And Lars~K. Rasmussen}

\aistatsaddress{ KTH Royal Institute of Technology } ]
...
\section{Introduction (\textcolor{red}{to be rewritten})}\label{sec:introduction}
...
\begin{equation}\label{eq:alpha-minimize-factor}
  \uargmin{\tilde{\phi}_{ts}^{\mathrm{new}}(x_t, x_s)}\hspace{-0.2cm}
...
\begin{align}\label{eq:message-rule}
  {m}^{\mathrm{new}}_{ts}(x_s) \propto  & m_{ts}(x_s)^{1-\alpha_{ts}} \bigg[
                                          \sum_{x_t} \phi_{ts}(x_t, x_s)^{\alpha_{ts}} {m}_{st}(x_t)^{1-\alpha_{ts}} \nonumber \\
...
\begin{proof}
  ...
  \begin{align}\label{eq:ratio_cvg_step}
    ...
    & \leq \abs{ 1 - \alpha_{ts}} \abs{z_{ts}^{(n)} - z_{ts}^{(n-1)}} \nonumber \\
  ...
  \begin{align}
    \normd{\bm{z}^{(n+1)} - \bm{z}^{n}} &\leq \normd{\bm{M}(\alpha, \bm{\theta})(\bm{z}^{(n)} - \bm{z}^{n-1})} \nonumber \\
...
\begin{cor}
  ...
  \begin{align}
    ...
    &+ \sum_{w \Nn(v) \backslash u} \tanh{\abs{\alpha_{{vw}} \theta_{vw}}} < 1,
  \end{align}
  or
  \begin{align}
