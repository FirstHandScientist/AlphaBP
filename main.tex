...
\begin{equation}
  \abs{\bm{z}^{(n+1)} - \bm{z}^{n}} \leq \bm{M}(\alpha, \theta)\abs{\bm{z}^{(n)} - \bm{z}^{n-1}}.
\end{equation}
If the spectral radius of $\bm{M}$ is less than $1$, the sequence $\abs{\bm{z}^{(n+1)} - \bm{z}^{(n)}}$ converges to zero. The sequence $\left\{ \bm{z}^{(n)} \right\}$ is a Cauchy sequence and must converge.
