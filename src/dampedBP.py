...
class dampedGraph(Graph):
    def __init__(self, eta=0.5, debug=DEBUG_DEFAULT):
        super(dampedGraph, self).__init__(debug=DEBUG_DEFAULT)
        # add now
        self.debug = debug
        self._rvs = {}
    ...
    def factor(self, rvs, name='', potential=None, meta={},
        ...
        # factor is redefined below
        f = Factor(rvs, self._eta, name, potential, meta, debug)
        self.add_factor(f)
        return f


class Factor(bpFactor):
    def __init__(self, rvs, eta, name='', potential=None, meta={},
                 debug=DEBUG_DEFAULT):
    ...
    def recompute_outgoing(self, normalize=False):
        ...
        for i, rv in enumerate(self._rvs):
            ...
            # for damped message update
            # self._outgoing[i] = o
            self._outgoing[i] = np.power(o, 1-self._eta) * \
                np.power(self._outgoing[i], self._eta)
