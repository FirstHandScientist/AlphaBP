...
class tree_reweight_Graph(Graph):
    def __init__(self, weight=1, debug=DEBUG_DEFAULT):
        super(tree_reweight_Graph, self).__init__(debug=DEBUG_DEFAULT)
        # add now
        self.debug = debug
        self._rvs = {}

        self._factors = []
        # edge appearance rate in the tree_reweighted graphs
        self._weight = weight
    ...
    def factor(self, rvs, name='', potential=None, meta={},
        ...
        f = Factor(rvs, self._weight, name, potential, meta, debug)
        self.add_factor(f)
        return f
...
class Factor(bpFactor):
    def __init__(self, rvs, weight, name='', potential=None, meta={},
                 debug=DEBUG_DEFAULT):
    ...
    def recompute_outgoing(self, normalize=False):
        ...
        for i, rv in enumerate(self._rvs):
            ...
            proj[i] = -1
            m_proj = np.power(m.reshape(proj), self._weight)
            # to exclude the over counting for unary factor
            if len(self.get_rvs()) > 1:
                m_proj = m_proj * np.power(rv.get_unary_incoming().reshape(proj), 1 - self._weight)
            fm_proj = np.power(fm.reshape(proj), self._weight - 1)
